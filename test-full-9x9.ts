import { generateCard, validateCard, checkRowWin, getCardNumbers } from './lib/game';
...
for (let row = 0; row < 9; row++) {
  ...
  for (let col = 0; col < 9; col++) {
    ...
    line += cell === null ? '    │' : ' ' + cell.toString().padStart(2, ' ') + ' │';
  }
  console.log(line);
  if (row < 8) {
    console.log('├────┼────┼────┼────┼────┼────┼────┼────┼────┤');
...
console.log(`   ${winningRows.includes(0) ? '✅ Row 0 detected as winner!' : '❌ Win detection failed'}`);
